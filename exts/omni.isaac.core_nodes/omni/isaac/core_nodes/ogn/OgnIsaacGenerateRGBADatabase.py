...
class OgnIsaacGenerateRGBADatabase(og.Database):
    ...
    class abi:
        ...
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnIsaacGenerateRGBADatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnIsaacGenerateRGBADatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnIsaacGenerateRGBADatabase._release_per_node_instance_data(node, graph_instance_id)
