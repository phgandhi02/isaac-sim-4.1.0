...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        from omni.isaac.core_nodes.ogn.OgnIsaacSetCameraOnRenderProductDatabase import OgnIsaacSetCameraOnRenderProductDatabase
        test_file_name = "OgnIsaacSetCameraOnRenderProductTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_omni_isaac_core_nodes_IsaacSetCameraOnRenderProduct")
        database = OgnIsaacSetCameraOnRenderProductDatabase(test_node)
