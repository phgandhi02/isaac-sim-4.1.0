...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        test_file_name = "OgnROS2ServicePrimTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_omni_isaac_ros2_bridge_ROS2ServicePrim")
        ...
        attribute = test_node.get_attribute("inputs:getAttributeServiceName")
        self.assertTrue(attribute.is_valid())
        expected_value = "get_prim_attribute"
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))

        self.assertTrue(test_node.get_attribute_exists("inputs:getAttributesServiceName"))
        attribute = test_node.get_attribute("inputs:getAttributesServiceName")
        self.assertTrue(attribute.is_valid())
        expected_value = "get_prim_attributes"
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
        ...
        attribute = test_node.get_attribute("inputs:primsServiceName")
        self.assertTrue(attribute.is_valid())
        expected_value = "get_prims"
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
        ...
        attribute = test_node.get_attribute("inputs:setAttributeServiceName")
        self.assertTrue(attribute.is_valid())
        expected_value = "set_prim_attribute"
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))
