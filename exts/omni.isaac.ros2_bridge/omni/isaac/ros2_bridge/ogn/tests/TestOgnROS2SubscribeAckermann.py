...
class TestOgn(ogts.OmniGraphTestCase):
    ...
    async def test_data_access(self):
        test_file_name = "OgnROS2SubscribeAckermannTemplate.usda"
        usd_path = os.path.join(os.path.dirname(__file__), "usd", test_file_name)
        if not os.path.exists(usd_path):  # pragma: no cover
            self.assertTrue(False, f"{usd_path} not found for loading test")
        (result, error) = await ogts.load_test_file(usd_path)
        self.assertTrue(result, f'{error} on {usd_path}')
        test_node = og.Controller.node("/TestGraph/Template_omni_isaac_ros2_bridge_ROS2SubscribeAckermannDrive")
        ...
        self.assertTrue(attribute.is_valid())
        expected_value = "ackermann_cmd"
        actual_value = og.Controller.get(attribute)
        ogts.verify_values(expected_value, actual_value, _attr_error(attribute, True))

        self.assertTrue(test_node.get_attribute_exists("outputs:acceleration"))
        attribute = test_node.get_attribute("outputs:acceleration")
        self.assertTrue(attribute.is_valid())
        ...
        self.assertTrue(test_node.get_attribute_exists("outputs:jerk"))
        attribute = test_node.get_attribute("outputs:jerk")
        self.assertTrue(attribute.is_valid())

        self.assertTrue(test_node.get_attribute_exists("outputs:speed"))
        attribute = test_node.get_attribute("outputs:speed")
        self.assertTrue(attribute.is_valid())

        self.assertTrue(test_node.get_attribute_exists("outputs:steeringAngle"))
        attribute = test_node.get_attribute("outputs:steeringAngle")
        self.assertTrue(attribute.is_valid())

        self.assertTrue(test_node.get_attribute_exists("outputs:steeringAngleVelocity"))
        attribute = test_node.get_attribute("outputs:steeringAngleVelocity")
        self.assertTrue(attribute.is_valid())
