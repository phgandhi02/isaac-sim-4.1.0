"""Support for simplified access to data on nodes of type omni.isaac.ros_bridge.ROS1CameraHelper
...
class OgnROS1CameraHelperDatabase(og.Database):
    """Helper class providing simplified access to data on nodes of type omni.isaac.ros_bridge.ROS1CameraHelper
    ...
    INTERFACE = og.Database._get_interface([
        ...
        ('inputs:frameId', 'string', 0, None, 'FrameId for ROS1 message, the nodeNamespace will be prefixed to the frame id', {ogn.MetadataKeys.DEFAULT: '"sim_camera"'}, True, "sim_camera", False, ''),
    ...
    class ValuesForInputs(og.DynamicAttributeAccess):
        LOCAL_PROPERTY_NAMES = {"enableSemanticLabels", "enabled", "execIn", "frameId", "frameSkipCount", "nodeNamespace", "queueSize", "renderProductPath", "resetSimulationTimeOnStop", "semanticLabelsTopicName", "stereoOffset", "topicName", "type", "useSystemTime", "viewport", "_setting_locked", "_batchedReadAttributes", "_batchedReadValues"}
        """Helper class that creates natural hierarchical access to input attributes"""
        def __init__(self, node: og.Node, attributes, dynamic_attributes: og.DynamicAttributeInterface):
            """Initialize simplified access for the attribute data"""
            context = node.get_graph().get_default_graph_context()
            super().__init__(context, node, attributes, dynamic_attributes)
            self._batchedReadAttributes = [self._attributes.enableSemanticLabels, self._attributes.enabled, self._attributes.execIn, self._attributes.frameId, self._attributes.frameSkipCount, self._attributes.nodeNamespace, self._attributes.queueSize, self._attributes.renderProductPath, self._attributes.resetSimulationTimeOnStop, self._attributes.semanticLabelsTopicName, self._attributes.stereoOffset, self._attributes.topicName, self._attributes.type, self._attributes.useSystemTime, self._attributes.viewport]
            self._batchedReadValues = [False, True, None, "sim_camera", 0, "", 10, "", False, "semantic_labels", [0, 0], "rgb", "rgb", False, ""]
    ...
    def __init__(self, node):
        ...
        self.inputs = OgnROS1CameraHelperDatabase.ValuesForInputs(node, self.attributes.inputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_OUTPUT)
        self.outputs = OgnROS1CameraHelperDatabase.ValuesForOutputs(node, self.attributes.outputs, dynamic_attributes)
        dynamic_attributes = self.dynamic_attribute_data(node, og.AttributePortType.ATTRIBUTE_PORT_TYPE_STATE)
        self.state = OgnROS1CameraHelperDatabase.ValuesForState(node, self.attributes.state, dynamic_attributes)
    ...
    class abi:
        ...
        @staticmethod
        def get_node_type():
            get_node_type_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'get_node_type', None)
            if callable(get_node_type_function):  # pragma: no cover
                return get_node_type_function()
            return 'omni.isaac.ros_bridge.ROS1CameraHelper'
        ...
        def compute(context, node):
            def database_valid():
                return True
            try:
                per_node_data = OgnROS1CameraHelperDatabase.PER_NODE_DATA[node.node_id()]
                db = per_node_data.get('_db')
                if db is None:
                    db = OgnROS1CameraHelperDatabase(node)
            ...
            try:
                compute_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'compute', None)
                ...
                with og.in_compute():
                    return OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS.compute(db)
        ...
        def initialize(context, node):
            OgnROS1CameraHelperDatabase._initialize_per_node_data(node)
            initialize_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'initialize', None)
        ...
        @staticmethod
        def release(node):
            release_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'release', None)
            if callable(release_function):  # pragma: no cover
                release_function(node)
            OgnROS1CameraHelperDatabase._release_per_node_data(node)

        @staticmethod
        def init_instance(node, graph_instance_id):
            init_instance_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'init_instance', None)
            if callable(init_instance_function):  # pragma: no cover
                init_instance_function(node, graph_instance_id)

        @staticmethod
        def release_instance(node, graph_instance_id):
            release_instance_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'release_instance', None)
            if callable(release_instance_function):  # pragma: no cover
                release_instance_function(node, graph_instance_id)
            OgnROS1CameraHelperDatabase._release_per_node_instance_data(node, graph_instance_id)

        @staticmethod
        def update_node_version(context, node, old_version, new_version):
            update_node_version_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'update_node_version', None)
        ...
        def initialize_type(node_type):
            initialize_type_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'initialize_type', None)
            needs_initializing = True
            if callable(initialize_type_function):  # pragma: no cover
                needs_initializing = initialize_type_function(node_type)
            if needs_initializing:
                node_type.set_metadata(ogn.MetadataKeys.EXTENSION, "omni.isaac.ros_bridge")
                node_type.set_metadata(ogn.MetadataKeys.UI_NAME, "ROS1 Camera Helper")
                ...
                icon_path = icon_path + '/' + "ogn/icons/omni.isaac.ros_bridge.ROS1CameraHelper.svg"
                node_type.set_metadata(ogn.MetadataKeys.ICON_PATH, icon_path)
                OgnROS1CameraHelperDatabase.INTERFACE.add_to_node_type(node_type)

        @staticmethod
        def on_connection_type_resolve(node):
            on_connection_type_resolve_function = getattr(OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS, 'on_connection_type_resolve', None)
    ...
    def register(node_type_class):
        OgnROS1CameraHelperDatabase.NODE_TYPE_CLASS = node_type_class
        og.register_node_type(OgnROS1CameraHelperDatabase.abi, 2)

    @staticmethod
    def deregister():
        og.deregister_node_type("omni.isaac.ros_bridge.ROS1CameraHelper")
