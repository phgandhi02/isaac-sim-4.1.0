...
class TestCase(PhysicsKitStageAsyncTestCase):
    ...
    async def do_visual_test(
        ...
        diff = await self.capture_and_compare(self.get_img_name(img_name, img_suffix), threshold, compare_op, self._viewport_data, use_renderer_capture)
        ...
        if not skip_assert:
            self.assertTrue(res, f"The image doesn't match the golden one: {self.get_img_name(img_name, img_suffix)}")
        return res
    ...
    async def capture(self, image_res_path, viewport_data, use_renderer_capture=False):
        import omni

        if image_res_path in self._image_paths:
            carb.log_error(f"Capture target {image_res_path} was already used for this test. Don't reuse image names, artifact publishing process on CI/CD can fight over it with next capture!")
        else:
        ...
        async def check_if_file_exists():
            if image_path.exists():
                print(f"{image_path} exists immediately")
            else:
                exists = False
                for i in range(0, exists_timeout):
                    ...
                    await omni.kit.app.get_app().next_update_async()
                if exists:
                    print(f"{image_path} appeared in {i} frames")
                else:
        ...
        if viewport_data and not use_renderer_capture:
            ...
            try:
                ...
                print(f"{image_path} captured in {elapsed_time}")
    ...
    async def capture_and_compare(self, image_name, threshold, compare_op, viewport_data, use_renderer_capture = False):
        ...
        await self.capture(image_res_path, viewport_data, use_renderer_capture)
