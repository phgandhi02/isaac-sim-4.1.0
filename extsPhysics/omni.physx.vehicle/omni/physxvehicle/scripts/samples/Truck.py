...
class ControlsNotificationHandler:
    ...
    def _on_simulation_event(self, event):
        if event.type == int(SimulationEvent.RESUMED):
            self._appUpdateEventSubscription = omni.kit.app.get_app().get_update_event_stream().create_subscription_to_pop(
                self._on_update, name="vehicle sample truck update"
            )
