...
class PhysicsBaseAsyncTestCase(omni.kit.test.AsyncTestCase, utils.AssertTestUtils):
    async def setUp(self):
        ...
        self._backward_compat = isregistry.get_as_bool(SETTING_BACKWARD_COMPATIBILITY)
        isregistry.set_int(SETTING_BACKWARD_COMPATIBILITY, BackwardCompatMode.WARN)
        self._sub_filter = isregistry.get_as_string("/exts/omni.physx.tests/subTestsFilter")
        self._remote_cache_was_on = isregistry.get_as_bool(SETTING_UJITSO_REMOTE_CACHE)
        isregistry.set_bool(SETTING_UJITSO_REMOTE_CACHE, False)
    ...
    async def tearDown(self):
        ...
        isregistry.set_int(SETTING_BACKWARD_COMPATIBILITY, self._backward_compat)
        isregistry.set_bool(SETTING_UJITSO_REMOTE_CACHE, self._remote_cache_was_on)
...
class PhysicsMemoryStageBaseAsyncTestCase(PhysicsBaseAsyncTestCase):
    ...
    async def new_stage(self, def_up_and_mpu=True, attach_stage=True, file_to_load=""):
