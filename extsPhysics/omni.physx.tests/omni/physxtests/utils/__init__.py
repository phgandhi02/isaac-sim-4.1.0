...
async def new_stage_setup(def_up_and_mpu=True):
...
def get_tests_asset_path(resource_relative_path: str) -> str:
    """
    ...
    Example:
        ...
        asset_path = get_tests_asset_path("common/sphere_high.usd")
    """
    assets_path_setting = carb.settings.get_settings().get(pb.SETTING_TESTS_ASSETS_PATH)
    assert assets_path_setting, "Empty string in setting /physics/testsAssetsPath!"
...
async def wait_for_stage_loading_status(usd_context=None, max_loops=2):
    ...
    while loops < max_loops:
        ...
        print(f"files loaded {files_loaded}, total_files {total_files}")
        await omni.kit.app.get_app().next_update_async()
