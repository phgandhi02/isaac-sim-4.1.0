...
class PhysxParticleClothAPITestAsync(PhysicsBaseAsyncTestCase):
    ...
    async def _wait_for_particle_cloth_cooking(self, max_updates: int = 100):
        cooking_iface = get_physx_cooking_interface()
        await omni.kit.app.get_app().next_update_async()
        #wait for particle cloth cooking tasks to finish
    ...
    async def _run_recooking_test(self, change_type: str, sync_type: str):
        ...
        #cook
        await self._wait_for_particle_cloth_cooking(max_updates=len(mesh_list)*100)
    ...
    async def test_particle_cloth_trigger_recooking_mesh_during_play(self):
        ...
        await self._wait_for_particle_cloth_cooking(max_updates=100)
    ...
    async def helper_particle_cloth_inconsistent_setup(self):
        ...
        await self._wait_for_particle_cloth_cooking()
    ...
    async def test_particle_cloth_inconsistent_setup(self):
        ...
        message = f"PhysxSchemaPhysxParticleClothAPI:restPoints and UsdGeomMesh:points have inconsistent sizes."
        mesh, particle_cloth = await self.helper_particle_cloth_inconsistent_setup()
        with utils.ExpectMessage(self, message, expected_result=True):
