...
class UtilsTestKitStage(PhysicsKitStageAsyncTestCase):
    category = TestCategory.Core

    def _has_cooking_data(self, stage):
        data = [t for t in omni.physx.scripts.utils.get_cooked_data_attributes(stage)]
        return len(data) > 0

    async def test_utils_release_cooked_data(self):
        stage = await self.new_stage()

        meshCube = utils.create_mesh_cube(stage, "/meshCube0", 20.0)
        UsdPhysics.CollisionAPI.Apply(meshCube.GetPrim())

        # step to cook and check
        await self.step(5)
        self.assertTrue(self._has_cooking_data(stage))

        # release and recheck
        omni.physx.scripts.utils.release_cooked_data(stage)
        self.assertFalse(self._has_cooking_data(stage))

    async def test_utils_do_not_save_cooked_data(self):
        stage = await self.new_stage()
        settings = carb.settings.get_settings()
        # Disable saving cooked data
        ...
        # step to cook and check
        await self.step(5)
        # Should not have any cooked data now
        self.assertFalse(self._has_cooking_data(stage))
        settings.set_bool(physx_bindings.SETTING_SAVE_COOKED_DATA, True)


    async def test_utils_do_not_save_cooked_data_on_session_layer(self):
        stage = await self.new_stage()
