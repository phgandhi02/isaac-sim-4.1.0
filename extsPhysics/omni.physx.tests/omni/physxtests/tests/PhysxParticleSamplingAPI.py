...
class PhysxParticleSamplingAPITestKitStage(PhysicsKitStageAsyncTestCase):
    ...
    async def wait_for_sampling_to_finish(self):
        # wait at least one tick for the update handler to pick up tasks
        await omni.kit.app.get_app().next_update_async() 

        update_count = 0
        # get_num_collision_tasks has a misleading name, but this checks if there are
        # any outstanding cooking tasks.
        while (get_physx_cooking_interface().get_num_collision_tasks() != 0):
            await omni.kit.app.get_app().next_update_async()
            update_count = update_count + 1
            self.assertLess(update_count, 10000)
    ...
    #@unittest.skip("https://nvidia-omniverse.atlassian.net/browse/OM-49894")
    async def test_particleSampling_sample_particles(self):
        ...
        await self.wait_for_sampling_to_finish()
