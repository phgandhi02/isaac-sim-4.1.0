...
class PrettyPrintTokenComboBuilder(TfTokenAttributeModelWMetadata):
    ...
    def _update_allowed_token(self):
        ...
        for n, t in enumerate(self._get_allowed_tokens(attr)):
            self._allowed_tokens.append(AllowedTokenItem(t, self._names[n]))

        for t, n in self._additions:
            self._allowed_tokens.append(AllowedTokenItem(t, n))
