...
from pxr import UsdPhysics, Usd, Sdf, UsdShade, UsdGeom, PhysxSchema, Gf
...
from .utils import get_align_property_util, is_parent_api_instance
from .externals import Prompt
from .models import TfTokenAttributeModelWMetadata
...
def get_components(payload, schema=None):
    ...
    for c in database.components.values():
        if schema is not None and c.main_schema != schema:
            continue
...
class PhysicsWidget(UsdPropertiesWidget):
    def __init__(self, title, schema, builders):
        ...
        self._is_main_multi = is_multi_api(schema)
        self._ext_apis = database.extension_api_map.get(self._main_schema, [])
        self._all_schemas = [self._main_schema] + database.widget_extension_map.get(self._main_schema, [])
        self._builders = builders
    ...
    def _customize_props_layout(self, props):
        return sort_props(props, self._all_schemas)
            ...
            if prop_name in self._single_schema_prop_names_set:
                ...
                if prop_spec is None:
                    def get_name():
                        ...
                        return f"a {name.__name__}" if name else "an unknown API"
    ...
    def build_impl(self):
        self._button_frame = None
        component = database.components.get(self._main_schema.__name__)
        ...
        def build_widget():
            ...
            if len(available_builders) != 0:
                ...
                if UsdPropertiesWidgetBuilder._get_type_name(prop.metadata).type == Sdf.ValueTypeNames.Token.type:
                    widget_kwargs["model_cls"] = TfTokenAttributeModelWMetadata
            ...
            def create_placeholder_attributes(prop, model, orig_fn, attributes):
                ...
                for attribute in attributes:
                    if prop.instance_name is not None:
                        prop.apply_schema.Apply(attribute.GetPrim(), prop.instance_name)
                    else:
                        prop.apply_schema.Apply(attribute.GetPrim())
    ...
    def _build_group_frame(self, name, props, stage):
        ...
        if self._is_main_multi:
            instance_name = f"{self._main_schema.__name__}:{props[0].instance_name}"
...
class MassAPIWidget(PhysicsWidget):
    ...
    def on_new_payload(self, payload):
        ...
        for prim_path in payload:
            ...
            hasHair = prim.HasAPI(PhysxSchema.PhysxHairAPI)
            ...
            if not hasRigidBody and not prim.HasAPI(UsdPhysics.CollisionAPI) and not hasDeformableBody and not hasDeformableSurface and not hasParticle and not hasHair:
                return False
...
class APIInheritanceCheckWidget(PhysicsWidget):
    ...
    def _build_group_frame(self, name, props, stage):
        if is_parent_api_instance(self._main_schema.__name__, props[0].instance_name, self._payload):
            return None
...
class ExtendedColliderWidget(PhysicsWidget):
    ...
    def _filter_props_to_build(self, prim):
        ...
        def try_inactivate(api, mesh_api, name):
            approximation = mesh_api.GetApproximationAttr().Get()
            if approximation != name or not prim.HasAPI(api):
                for prop_spec in self._ext_prop_specs[api]:
        ...
        if mesh_api:
            for name, api in utils.MESH_APPROXIMATIONS.items():
                if api is not None:
                    try_inactivate(api, mesh_api, name)
        return filtered_props
...
class ExtendedFEMMaterialWidget(PhysicsWidget):
    def _filter_props_to_build(self, prim):
        filtered_props = super()._filter_props_to_build(prim)
        trim_list = []

        if prim.HasAPI(PhysxSchema.PhysxSoftBodyAPI):
            trim_list = ["physxFEMMaterial:thickness", "physxFEMMaterial:selfCollisionFilterDistance", "physxFEMMaterial:mu",
                         "physxFEMMaterial:lambda", "physxFEMMaterial:bendingScale", "physxFEMMaterial:elasticityScale",
                         "physxFEMMaterial:areaConstraintScale", "physxFEMMaterial:restAreaInflation", "physxFEMMaterial:velocityDamping",
                         "physxFEMMaterial:areaDamping", "physxFEMMaterial:elasticityDamping", "physxFEMMaterial:bendingDamping",
                         "physxFEMMaterial:clothModel"]

        if prim.HasAPI(PhysxSchema.PhysxFEMClothAPI):
            trim_list = ["physxFEMMaterial:damping", "physxFEMMaterial:enableDeformation", "physxFEMMaterial:deformThreshold",
                         "physxFEMMaterial:deformLowLimitRatio", "physxFEMMaterial:deformHighLimitRatio"]

        for prop_name in trim_list:
            prop = prim.GetProperty(prop_name)
            filtered_props.remove(UiProp().from_property(prop))
...
class InvisibleWidget(InvisibleMenuWidgetBase):
    ...
    def _undo_redo_on_change(self, cmds):
        if any(item in [
            "ApplyAPISchema", "UnapplyAPISchema", "AddDeformableBodyComponent", "AddDeformableSurfaceComponent",
            "AddPhysicsComponent", "RemovePhysicsComponent",
...
class PhysicsMaterialBindingWidget(UsdBindingAttributeWidget):
    ...
    def on_new_payload(self, payload):
        ...
        for path in payload:
            ...
            if not prim or (not prim.HasAPI(UsdPhysics.CollisionAPI) and not prim.HasAPI(PhysxSchema.PhysxDeformableBodyAPI) and not prim.HasAPI(PhysxSchema.PhysxDeformableSurfaceAPI) and not prim.HasAPI(PhysxSchema.PhysxHairAPI)):
                return False
    ...
    def _filter_props_to_build(self, prim):
        ...
        if prim.HasAttribute("physxDeformable:kinematicEnabled"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:kinematicEnabled",
                                  ExtendedDeformableBodyWidget.is_kinematic_title, "", 'bool', kinematicEnabled))
        if prim.HasAttribute("physxDeformable:simulationHexahedralResolution"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:simulationHexahedralResolution",
                                  ExtendedDeformableBodyWidget.sim_mesh_res_title, "", 'int', simulationHexahedralResolution))
        if prim.HasAttribute("physxDeformable:collisionSimplification"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:collisionSimplification",
                                  ExtendedDeformableBodyWidget.coll_simp_title, "", 'bool', collisionSimplification))
        if prim.HasAttribute("physxDeformable:collisionSimplificationRemeshing"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:collisionSimplificationRemeshing",
                                  ExtendedDeformableBodyWidget.coll_simp_remeshing_title, "", 'bool', collisionSimplificationRemeshing))
        if prim.HasAttribute("physxDeformable:collisionSimplificationRemeshingResolution"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:collisionSimplificationRemeshingResolution",
                                  ExtendedDeformableBodyWidget.coll_simp_remeshing_res_title, "", 'int', collisionSimplificationRemeshingResolution))
        if prim.HasAttribute("physxDeformable:collisionSimplificationTargetTriangleCount"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:collisionSimplificationTargetTriangleCount",
                                  ExtendedDeformableBodyWidget.coll_simp_target_title, "", 'int', collisionSimplificationTargetTriangleCount))
        if prim.HasAttribute("physxDeformable:collisionSimplificationForceConforming"):
            ...
            filtered_props.append(UiProp().from_custom("physxDeformable:collisionSimplificationForceConforming",
                                  ExtendedDeformableBodyWidget.coll_simp_force_conforming_title, "", 'bool', collisionSimplificationForceConforming))
    ...
    def _build_property_item(self, stage, prop, prim_paths):
        ...
        if prop.base_name == "physxDeformable:simulationHexahedralResolution":
            if not model._ambiguous and (model.as_int < prop.info.min or model.as_int > prop.info.max):
                with ui.HStack():
                    UsdPropertiesWidgetBuilder._create_label(ExtendedDeformableBodyWidget.sim_mesh_res_title)
                    ui.Spacer(width=8)
...
class ExtendedComputeAutoAttachmentWidget(PhysicsWidget):
    ...
    prop_data_custom = {
        # defaults for min/MaxHairRoots mirrored in CreatePhysicsAttachmentCommand and PhysxAutoAttachmentDesc
        "physxAutoAttachment:minHairRoots" : ("Min Hair Roots", "", 'uint', 3, "Minimal number of hair root particles that are attached"),
        "physxAutoAttachment:maxHairRoots" : ("Max Hair Roots", "", 'uint', Limits.INT_MAX, "Maximal number of hair root particles that are attached"),
    ...
    def is_collisionFilteringOffset_enabled(self):
        return self._bool_controllers["physxAutoAttachment:enableCollisionFiltering"] and self._none_have_type[self.AType.HAIR] and self._none_have_type[self.AType.WORLD]

    def is_hair_root_limits_enabled(self):
        return self._all_have_type[self.AType.HAIR]
                ...
                for t in range(2):
                    ...
                    if len(targets) > 0:
                        ...
                        if target_prim:
                            ...
                            has_type[self.AType.HAIR] = has_type[self.AType.HAIR] or target_prim.HasAPI(PhysxSchema.PhysxHairAPI)
    ...
    def _filter_props_to_build(self, prim):
        ...
        minHairRootsAttr = prim.CreateAttribute("physxAutoAttachment:minHairRoots", Sdf.ValueTypeNames.UInt)
        maxHairRootsAttr = prim.CreateAttribute("physxAutoAttachment:maxHairRoots", Sdf.ValueTypeNames.UInt)
    ...
    def _build_property_item(self, stage, prop, prim_paths):
        ...
        if prop.base_name == "physxAutoAttachment:minHairRoots" or prop.base_name == "physxAutoAttachment:maxHairRoots":
            self._make_state_controlled(prop.base_name, model, "is_hair_root_limits_enabled")
    ...
    def _filter_props_to_build(self, prim):
        ...
        if prim.HasAttribute("physxParticle:springStretchStiffness"):
            filtered_props.append(UiProp().from_custom("physxParticle:springStretchStiffness",
                                  ExtendedAutoParticleClothWidget.springStretchStiffness_title, "", 'float', True))
        ...
        if prim.HasAttribute("physxParticle:springBendStiffness"):
            filtered_props.append(UiProp().from_custom("physxParticle:springBendStiffness",
                                  ExtendedAutoParticleClothWidget.springBendStiffness_title, "", 'float', True))
        ...
        if prim.HasAttribute("physxParticle:springShearStiffness"):
            filtered_props.append(UiProp().from_custom("physxParticle:springShearStiffness",
                                  ExtendedAutoParticleClothWidget.springShearStiffness_title, "", 'float', True))

        if prim.HasAttribute("physxParticle:springDamping"):
            filtered_props.append(UiProp().from_custom("physxParticle:springDamping",
                                  ExtendedAutoParticleClothWidget.springDamping_title, "", 'float', True))
