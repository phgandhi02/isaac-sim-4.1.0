...
class AddHairCommand(Command):
    ...
    def do(self) -> bool:
        ...
        prim = stage.GetPrimAtPath(self._prim_path)
        if not prim:
            carb.log_error(type(self).__name__ + ": prim_path not pointing to valid Usd.Prim")
            return False

        if not UsdGeom.Mesh(prim) and not UsdGeom.BasisCurves(prim) and not UsdGeom.Subset(prim):
            carb.log_error(type(self).__name__ + ": prim_path not pointing to valid UsdGeom.Mesh, UsdGeom.BasisCurves, or UsdGeom.Subset")
            return False

        self._usd_undo = UsdLayerUndo(stage.GetEditTarget().GetLayer())
        self._usd_undo.reserve(self._prim_path)

        # Warn if prim's pose is subject to a time-varying transform
        if (not UsdGeom.Xformable(prim).GetResetXformStack()):
            needsResetXformStack = False
            currPrim = prim
            while(not needsResetXformStack and currPrim != stage.GetPseudoRoot()):
                needsResetXformStack |= UsdGeom.Xformable(currPrim).TransformMightBeTimeVarying()
                needsResetXformStack |= currPrim.HasAPI(UsdPhysics.RigidBodyAPI)
                currPrim = currPrim.GetParent()

            if(needsResetXformStack):
                carb.log_warn(type(self).__name__ + ": When adding hair system to prim the local-to-world transforms must be not be time varying. The simulation may be behave in unexpected ways.")

        physxHairApi = PhysxSchema.PhysxHairAPI.Apply(prim)
        success = physxHairApi is not None
        if not success:
            self.undo()
...
class RemoveHairCommand(Command):
    ...
    def do(self):
        ...
        # Remove attributes because UnapplyAPISchemaCommand does not remove them
        usdPrim = stage.GetPrimAtPath(self._prim_path)
        filtered = fnmatch.filter(usdPrim.GetPropertyNames(), 'physxHair*')
        for attr in filtered:
            RemoveAttributeCommand(attr, usdPrim).do()

        success = UnapplyAPISchemaCommand(PhysxSchema.PhysxHairAPI, usdPrim).do()
        if not success:
            self.undo()
    ...
    def do(self):
        ...
        if "PhysicsLimit" in self._component:
            ...
            if "trans" in axis:
                ...
                limitAPI.CreateLowAttr(-180.0)
                limitAPI.CreateHighAttr(180.0)
...
class AddPhysicsComponentCommand(Command):
    ...
    def do(self):
        ...
        elif self._component == "PhysxHairAPI":
            execute("AddHairCommand", prim_path=self._usd_prim.GetPath())
        elif self._component == "PhysxHairMaterialAPI":
            execute("ApplyAPISchema", api=PhysxSchema.PhysxHairMaterialAPI, prim=self._usd_prim)
...
class RemovePhysicsComponentCommand(Command):
    ...
    def do(self):
        ...
        if self._component == "PhysicsRigidBodyAPI":
            ...
            execute("RemoveHairCommand", prim_path=self._usd_prim.GetPath())
        elif self._component == "PhysxHairMaterialAPI":
            remove_api(PhysxSchema.PhysxHairMaterialAPI)
...
class CreatePhysicsAttachmentCommand(Command):
    ...
    def _create_attachment(self, target_attachment_path: Sdf.Path, actor0_path: Sdf.Path, actor1_path: Sdf.Path) -> bool:
        ...
        # defaults for min/MaxHairRoots mirrored in ExtendedComputeAutoAttachmentWidget and PhysxAutoAttachmentDesc
        minHairRootsAttr = attachment.GetPrim().CreateAttribute("physxAutoAttachment:minHairRoots", Sdf.ValueTypeNames.UInt)
        maxHairRootsAttr = attachment.GetPrim().CreateAttribute("physxAutoAttachment:maxHairRoots", Sdf.ValueTypeNames.UInt)
        if minHairRootsAttr:
            minHairRootsAttr.Set(3)
        if maxHairRootsAttr:
            # failed to from omni.kit.property.physx import utils as propertyUtils
            # maxHairRootsAttr.Set(propertyUtils.Limits.INT_MAX)
            INT_MAX = 2147483647
            maxHairRootsAttr.Set(INT_MAX)
...
class AddHairMaterialCommand(PhysicsCommand):
    """
    Adds a PhysxSchema.HairMaterialAPI to a UsdShade.Material prim, and creates the material prim if needed.
    ...
    def do(self):
        ...
        if not utils.ensureMaterialOnPath(self._stage, self._path):
            return False

        material = PhysxSchema.PhysxHairMaterialAPI.Apply(self._stage.GetPrimAtPath(self._path))
    ...
    def undo(self):
        if self._del_prim:
            ...
            prim = self._stage.GetPrimAtPath(self._path)
            UnapplyAPISchemaCommand(PhysxSchema.PhysxHairMaterialAPI, prim).do()
