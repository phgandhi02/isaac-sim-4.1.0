...
def _getSchemaOwner(prop, schema) -> Usd.SchemaBase:
    ...
    try:
        ...
        namespaces = [Usd, UsdPhysics, PhysxSchema, ForceFieldSchema]
    except ImportError:
...
class ExpectMessage():
    def __init__(self, test_case, expected: str | list[str], expected_result=True, expect_all=True, result_fn=None):
        """
    ...
    def __enter__(self):
        ...
        if type(self._expected) is list:
            get_physxunittests_interface().start_logger_check_for_multiple(self._expected, self._expected_result, self._expect_all)
        else:
            get_physxunittests_interface().start_logger_check(self._expected, self._expected_result)
...
def get_cooked_data_attributes(stage):
    attr_names = dict()
    for token in COOKED_DATA_TOKENS:
        attr_names[token] = PhysxSchema.PhysxCookedDataAPI.GetSchemaAttributeNames(False, token)

    for prim in stage.Traverse():
        for token, names in attr_names.items():
            has_api = PhysxSchema.PhysxCookedDataAPI.Get(prim, token)
            for name in names:
                if prim.HasAttribute(name):
                    yield (prim, token, name, has_api)


def release_cooked_data(stage):
    data = get_cooked_data_attributes(stage)
    for prim, token, name, has_api in data:
        prim.RemoveProperty(name)
        if has_api:
            prim.RemoveAPI(PhysxSchema.PhysxCookedDataAPI, token)
