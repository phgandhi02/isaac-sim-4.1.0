...
from .simulationDataVisualizer import SimulationDataVisualizerWindow
...
class PhysxViewportMenu:
    def __init__(self):
        ...
        self._simulation_data_visualizer_wnd = None
    ...
    def register_with_viewport(self):
        ...
        def _build_menu():
            ...
            def simulation_collision_store_setting(item, setting):
                self._helper.simulation_collision_store_setting(item, setting)
            ...
            def create_menu_sim_data_visualizer_bool_item(text, setting) -> CategoryStateItem:
                def show_overlay(model, item):
                    show = model.get_value_as_bool()
                    settings.set(setting, show)
                    if self._simulation_data_visualizer_wnd is None:
                        self._simulation_data_visualizer_wnd = SimulationDataVisualizerWindow() 
                        self._simulation_data_visualizer_wnd.set_pos(105, 150)
                    if show:
                        self._simulation_data_visualizer_wnd.show()
                    else:
                        self._simulation_data_visualizer_wnd.hide()
            ...
            with ui.Menu("Physics", delegate=ViewportMenuDelegate()):
                ...
                with ui.Menu("Deformable Body", delegate=ViewportMenuDelegate()):
                    ...
                    ui.Separator()
                    create_radioboxes_for_setting(physx_bindings.SETTING_DISPLAY_DEFORMABLE_BODY_TYPE, ["Simulation", "Collision"], simulation_collision_store_setting)
                ...
                with ui.Menu("Particles", delegate=ViewportMenuDelegate()):
                    ...
                    create_checkbox_for_setting("Cloth Particles", physx_bindings.SETTING_DISPLAY_PARTICLES_SHOW_CLOTH_PARTICLES)
    ...
    def unregister_from_viewport(self):
        ...
        if self._simulation_data_visualizer_wnd is not None:
            self._simulation_data_visualizer_wnd.undock()
            self._simulation_data_visualizer_wnd.destroy()
            self._simulation_data_visualizer_wnd=None
