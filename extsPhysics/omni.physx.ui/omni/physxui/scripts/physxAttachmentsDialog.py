...
def extend_range(bbox, percentage):
...
def is_overlap(stage, path0, path1, overlap_tolerance):
    ...
    range0 = extend_range(bbox0, overlap_tolerance)
    range1 = extend_range(bbox1, overlap_tolerance)
...
def is_hair(prim):
    return True if PhysxSchema.PhysxHairAPI(prim) else False

def is_rigid(prim):
    return True if (UsdPhysics.RigidBodyAPI(prim) or UsdPhysics.CollisionAPI(prim)) else False

def is_pair_valid(stage, path0, path1):
    prim0 = stage.GetPrimAtPath(path0)
    prim1 = stage.GetPrimAtPath(path1)
    result = True
    ...
    if is_hair(prim0) and is_hair(prim1):
        result = False
    ...
    if is_particle_cloth(prim0) and is_hair(prim1):
        result = False

    if is_hair(prim0) and is_particle_cloth(prim1):
        result = False

    if is_deformable_surface(prim0) and is_hair(prim1):
        result = False

    if is_hair(prim0) and is_deformable_surface(prim1):
        result = False       

    return result

def compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, actor0s, actor1s, overlap_tolerance):
    for path0 in actor0s:
        for path1 in actor1s:
            if path0 != path1 and not is_pair_exist(path0, path1, pair_dict) and is_pair_valid(stage, path0, path1) and is_overlap(stage, path0, path1, overlap_tolerance):
                if path0 not in pair_dict:
                    pair_dict[path0] = list()
                pair_dict[path0].append(path1)

def compute_overlapped_paths_pairs(stage, particlecloths, deformablebodies, deformablesurfaces, hair, rigids, overlap_tolerance):
    # Compute AABB overlap to determine potential actor pairs
    pair_dict = dict()
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, particlecloths, deformablebodies, overlap_tolerance)
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, particlecloths, rigids, overlap_tolerance)
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, deformablebodies, deformablebodies, overlap_tolerance)
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, deformablebodies, deformablesurfaces, overlap_tolerance)
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, deformablebodies, rigids, overlap_tolerance)
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, deformablesurfaces, rigids, overlap_tolerance)
    compute_overlapped_paths_pairs_from_two_actor_groups(stage, pair_dict, hair, rigids, overlap_tolerance)
...
class AttachmentsDialog:
    def __init__(self, particlecloths, deformablebodies, deformablesurfaces, hair, rigids):
        self._particlecloths = particlecloths
        self._deformablebodies = deformablebodies
        self._deformablesurfaces = deformablesurfaces
        self._hair = hair
        self._rigids = rigids
        ...
        self._pair_dict = compute_overlapped_paths_pairs(stage, particlecloths, deformablebodies, deformablesurfaces, hair, rigids, self._overlap_tolerance) 
    ...
    def _on_tolerance_changed(self, value):
        ...
        stage = self._usd_context.get_stage()
        self._pair_dict = compute_overlapped_paths_pairs(stage, self._particlecloths, self._deformablebodies, self._deformablesurfaces, self._hair, self._rigids, self._overlap_tolerance)
        self._sorted_pairs = self.sort_pairs_by_islands(self._pair_dict)
