...
def float_to_string(value, max_length = 12, precision=2) -> str:
    ...
    max_decimals = max_length - 1 # assume one is used for the decimal point.
    if value < 0.0:
        max_decimals - 1

    value = round(value, min(precision, max_decimals - 1 if max_decimals > 1 else 0))
    ...
    value_1k_separator = 1000.0
    while abs_value >= value_1k_separator:
        max_decimals -=1
        value_1k_separator *= 1000.0

    if abs_value >= math.pow(10.0, max_decimals-1): # or math.fabs(value) <= math.pow(10.0, -max_decimals+1):
        return ("{value:,>{max_length}.{decimals}e}").format(value=value, decimals=max_decimals - 3 if max_decimals > 4 else 1, max_length=max_length)
 
    return ("{value:,.{decimals}f}").format(value=value, decimals=min(precision, max_decimals if max_decimals > 0 else 1))
