...
from omni.physx.bindings._physx import SETTING_BACKWARD_COMPATIBILITY
...
def backward_compat_callback():
    backward_compat = carb.settings.get_settings().get(SETTING_BACKWARD_COMPATIBILITY)
    if(backward_compat != BackwardCompatMode.DISABLED and get_physx_interface().check_backwards_compatibility()):
        check_log = get_physx_interface().get_backwards_compatibility_check_log()
        if backward_compat == BackwardCompatMode.WARN:
            carb.log_warn(f"Deprecated physics schema format found!\nUpgrade your file manually or set Backward Compatibility Check in Physics Preferences to 'Ask' for an on-demand upgrade option or 'Auto-Upgrade' for an automatic upgrade whenever a deprecated schema is found (this operation might add, change and remove prims, APIs, properties etc.!).\nDeprecated schema log:\n{check_log}")
        elif backward_compat == BackwardCompatMode.ASK:
            text = "Deprecated physics schema format found. Do you want to run backward compatibility processing? This operation might add, change and remove prims, APIs, properties etc.!\nThis prompt can be turned off in Preferences."
            BackwardCompatPrompt(text, check_log).show()
        elif backward_compat == BackwardCompatMode.AUTO:
            carb.log_warn("Deprecated physics schema format found! Physics preferences are set to automatic upgrade.\nDeprecated schema log:\n{check_log}")
            get_physx_interface().run_backwards_compatibility()
            if get_physx_interface().check_backwards_compatibility():
                carb.log_warn(f"After upgrade log:\n{get_physx_interface().get_backwards_compatibility_check_log()}")
                carb.log_error(f"Backward compatibility processing unsuccessful. Please check log and try running our folder batch conversion tool through {FOLDER_NOTE}")
