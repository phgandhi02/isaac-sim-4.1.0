...
class CollisionGroupsWindow:
    ...
    def on_startup(self):
        ...
        self._stage_event_sub = omni.usd.get_context().get_stage_event_stream().create_subscription_to_pop(
            self._on_stage_event, name="omni.physx.ui.CollisionGroupsWindow._on_stage_event"
        )

        self.create_window()

        def main_menu_click():
            val = self._settings.get_as_bool(pb.SETTING_SHOW_COLLISION_GROUPS_WINDOW)
            self._settings.set_bool(pb.SETTING_SHOW_COLLISION_GROUPS_WINDOW, not val)
    ...
    def _on_stage_event(self, evt):
        if self._window is None or not self._window.visible:
            return
        if evt.type == int(omni.usd.StageEventType.OPENED) or evt.type == int(omni.usd.StageEventType.CLOSED):
    ...
    def _show_collision_groups_window_visibility_changed(self, item, event_type):
        if event_type == carb.settings.ChangeEventType.CHANGED:
            window_enabled = self._settings.get_as_bool(pb.SETTING_SHOW_COLLISION_GROUPS_WINDOW)
            if self._window:
                if window_enabled:
                    self._window.frame.rebuild() if self._window.frame is not None else self._build_ui()
                else:
    ...
    def _visiblity_changed_fn(self, visible):
        ...
        if not visible:
            window_enabled = self._settings.get_as_bool(pb.SETTING_SHOW_COLLISION_GROUPS_WINDOW)
            if window_enabled:
                self._settings.set_bool(pb.SETTING_SHOW_COLLISION_GROUPS_WINDOW, False)
        if self._menu:
            self._menu.refresh()
