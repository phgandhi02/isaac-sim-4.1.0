...
class PhysxUIViewportOverlays:
    ...
    def _check_vp2_expired_weak_ref(self, e: carb.events.IEvent):
        if self._viewport2_weak_ref is not None: # this tells us we are on VP2
            try:
                if self._viewport2_weak_ref:     # this checks validity of the weakref.proxy
                    pass
    ...
    def _attach_scene_view(self):
        ...
        with frame:
            ...
            with self._clip_vstack:
                ...
                with self._overlay_scene_view.scene:
                    ...
                    if not hasattr(viewport.viewport_api, 'legacy_window'):
        ...
        if self._viewport2_destroyed_poll_checking:
            if not hasattr(viewport, 'legacy_window'):
                self._viewport2_weak_ref = viewport
                if self._viewport2_check_expired_ref_sub is None:
                    # This subscription can be removed once we have an event to track VP2 destruction/recreation
                    # or when the change for hiding instead of destroying viewport will be ported to kit master/104
                    self._viewport2_check_expired_ref_sub = omni.kit.app.get_app().get_update_event_stream().create_subscription_to_pop(self._check_vp2_expired_weak_ref)
