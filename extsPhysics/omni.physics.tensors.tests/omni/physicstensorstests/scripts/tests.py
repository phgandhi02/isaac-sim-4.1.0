...
from omni.physxui.scripts.settings import BackwardCompatMode
...
class TestArticulationDofProperties(GridTestBase):
    ...
    def on_start(self, sim):
        ...
        max_velocities = np.ones((humanoids.count, humanoids.max_dofs)) * 1000
        all_indices = wp_utils.arange(humanoids.count)
...
class TestArticulationJointBodyOrderDofForce(TestArticulationJointBodyOrder):
    ...
    def on_physics_step(self, sim, stepno, dt):
        ...
        if stepno==4:
            ...
            self.test_case.assertTrue(np.allclose(dof_gravity_forces_1, dof_gravity_forces_2, rtol=1e-03, atol=self.atol), "similar generalized gravity forces")
...
class TestArticulationRootTransforms(TestArticulationRoots):
    ...
    def on_physics_step(self, sim, stepno, dt):
        if stepno == 1:
            ...
            lt_np = lt.numpy().reshape((self.ants.count, self.ants.max_links, 7))
            # print()
            # print(self.expected_root_transforms)
...
class TestGeneralizedGravity(GridTestBase):
    def __init__(self, test_case, device_params):
        # set up stage
        grid_params = GridParams(16, 2.5)
        sim_params = SimParams()
    ...
    def on_physics_step(self, sim, stepno, dt):
        ...
        self.ants.set_dof_actuation_forces(rc_forces, rc_indices)
        if stepno == 20:
            generalized_gravity = self.ants.get_generalized_gravity_forces()
            self.test_case.assertTrue(generalized_gravity.shape == (self.ants.count, self.ants.max_dofs))
            applied_efforts = self.ants.get_dof_actuation_forces()
            # print(np.dstack((applied_efforts.numpy(), rc_forces.numpy())))
            self.test_case.assertTrue(np.allclose(applied_efforts.numpy().flatten(), rc_forces.numpy(), rtol=1e-03, atol=1e-03), "expected actuation forces")
            self.finish()
...
class TestArticulationTendonProperties(GridTestBase):
    def __init__(self, test_case, device_params):
        # set up stage
        grid_params = GridParams(16, 2.0)
        sim_params = SimParams()
    ...
    def on_start(self, sim):
        ...
        stiffnesses = hands.get_fixed_tendon_stiffnesses().numpy()
        dampings = hands.get_fixed_tendon_dampings().numpy()
        limit_stiffnesses = hands.get_fixed_tendon_limit_stiffnesses().numpy()
        limits = hands.get_fixed_tendon_limits().numpy().reshape(hands.count, hands.max_fixed_tendons, 2)
        rest_lengths = hands.get_fixed_tendon_rest_lengths().numpy()
        offsets = hands.get_fixed_tendon_offsets().numpy()

        stiffnesses = wp.from_numpy(np.ones_like(stiffnesses) * 0.1, dtype=wp.float32, device=sim.device)
        dampings = wp.from_numpy(np.ones_like(dampings) * 0.2, dtype=wp.float32, device=sim.device)
        limit_stiffnesses = wp.from_numpy(np.ones_like(limit_stiffnesses) * 0.1, dtype=wp.float32, device=sim.device)
        limits[:, :, 0] = -0.1
        limits[:, :, 1] = 0.1
        limits = wp.from_numpy(limits, dtype=wp.float32, device=sim.device)
        rest_lengths = wp.from_numpy(np.ones_like(rest_lengths) * 0.2, dtype=wp.float32, device=sim.device)
        offsets = wp.from_numpy(np.ones_like(offsets) * 0.1, dtype=wp.float32, device=sim.device)
        ...
        # TODO: this won't work on GPU pipeline - would need a direct GPU API for read back
        # self.test_case.assertTrue(np.allclose(hands.get_fixed_tendon_stiffnesses().numpy(), stiffnesses))
        # self.test_case.assertTrue(np.allclose(hands.get_fixed_tendon_dampings().numpy(), dampings))
        # self.test_case.assertTrue(np.allclose(hands.get_fixed_tendon_limit_stiffnesses().numpy(), limit_stiffnesses))
        # self.test_case.assertTrue(np.allclose(hands.get_fixed_tendon_limits().numpy(), limits.flatten()))
        # self.test_case.assertTrue(np.allclose(hands.get_fixed_tendon_rest_lengths().numpy(), rest_lengths))
        # self.test_case.assertTrue(np.allclose(hands.get_fixed_tendon_offsets().numpy(), offsets))
...
class TestRigidContactMatrix(GridTestBase):
    ...
    def on_start(self, sim):
        ...
        sensor_pattern = [
            ...
            "/envs/env[1-3][0-9]/ball" # ball10-ball32
            ]
...
class TestUsdPrimDeletion(GridTestBase):
    ...
    def on_physics_step(self, sim, stepno, dt):
        ...
        if stepno == 2:
            # omni.kit.commands.execute("DeletePrims", paths=["/envs/env10/ball"])
            self.stage.RemovePrim("/envs/env2/ball")
            # self.stage.RemovePrim("/envs/env6/ant/torso")
            # calling any view class should return error message but shouldn't crash kit
            # self.rigid_body_view.get_transforms()
...
class PhysicsTensorsTests(PhysicsBaseAsyncTestCase):
    ...
    def setUpClass(cls):
        ...
        try:
            wp_utils.ensure_warp_msvc_compiler()
            cls.push_extensions()
        except:
            traceback.print_exc()
    ...
    async def test_generalized_gravity_cc(self):
        await self.run_test(TestGeneralizedGravity(self, DeviceParams(False, False)))

    async def test_generalized_gravity_gc(self):
        await self.run_test(TestGeneralizedGravity(self, DeviceParams(True, False)))

    async def test_generalized_gravity_gg(self):
        await self.run_test(TestGeneralizedGravity(self, DeviceParams(True, True)))
    ...
    async def test_articulation_tendon_properties_cc(self):
        await self.run_test(TestArticulationTendonProperties(self, DeviceParams(False, False)))

    async def test_articulation_tendon_properties_gc(self):
        await self.run_test(TestArticulationTendonProperties(self, DeviceParams(True, False)))

    async def test_articulation_tendon_properties_gg(self):
        await self.run_test(TestArticulationTendonProperties(self, DeviceParams(True, True)))
    ...
    @unittest.skip("https://nvidia-omniverse.atlassian.net/browse/OM-91738")
