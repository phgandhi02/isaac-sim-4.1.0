...
class LegoTechnicBuggy(demo.Base):
    title = "Lego Technic Buggy (Soft Tires)"
    category = demo.Categories.INTERNAL_SAMPLES
    short_description = "Joints, soft bodies and audio integrated into a single simulation of a Lego buggy"
    ...
    def setup_cameras(self):
        ...
        self._cameras_enabled = manager.is_extension_enabled("omni.physx.camera")

        if not self._cameras_enabled:
            manager.set_extension_enabled_immediate("omni.physx.camera", True)
    ...
    def create(self, stage):
        ...
        UsdGeom.SetStageMetersPerUnit(stage, 1) # we want 0.01 ideally but it currently explodes

        defaultPrimPath = str(stage.GetDefaultPrim().GetPath())

        # The assets are currently at:
        baseFolder = "omniverse://ov-content/Projects/DemoContent/DoNotDistribute/Physics_Preview_Demos/lego_technic/"
        scene_asset_path = baseFolder + "lego_technic_buggy_soft_tires3c.usd"
        self.audioPath = os.path.abspath("../../../data/audio/physics/vehicle")

        stage.GetDefaultPrim().GetReferences().AddReference(scene_asset_path, "/World")
    ...
    def draw_info_text(self):
        text = ["Press 1 to hide/show the chassis & engine block."]
        text.append("Press 2 to switch between camera views.")
    ...
    def update_steer_angle(self, dt):
        ...
        steeringHinge = UsdPhysics.DriveAPI.Get(self._steering_wheel_hinge, "angular")
        steeringHinge.GetTargetPositionAttr().Set(self.steerAngle * MAX_STEER_ANGLE)
