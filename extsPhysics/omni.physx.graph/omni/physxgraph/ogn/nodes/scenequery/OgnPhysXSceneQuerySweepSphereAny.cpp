...
#include <OgnPhysXSceneQuerySweepSphereAnyDatabase.h>
...
class OgnPhysXSceneQuerySweepSphereAny
{
public:
    static bool compute(OgnPhysXSceneQuerySweepSphereAnyDatabase& db)
    {
        // Read all inputs.
        carb::Float3 origin;
        {
        ...
        if(range != 0.0f)
        {
            bHit = getPhysXSceneQuery()->sweepSphereAny(radius, origin, direction, range < 0.0f ? PX_MAX_F32 : range, both_sides);
        }
