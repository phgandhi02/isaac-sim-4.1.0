...
struct omni::physx::graph::MeshIntersectionFacesResult
{
    ...
    std::unordered_set<uint32_t> triangles0; // cache variable used to filter multiple matches for the same triangle in
                                             // one mesh against other triangles in the other mesh
    std::unordered_set<uint32_t> triangles1; // same as above
    std::unordered_set<uint32_t> intersectingFaces0;
    std::unordered_set<uint32_t> intersectingFaces1;
...
struct omni::physx::graph::ImmediateMeshIntersectionFaces : public ImmediateNarrowPhase
{
                                             ...
                                             const MeshFacesToTrianglesMapping* meshFacesToTrianglesMapping1)
    {
        ...
        for (size_t triIDX = 0; triIDX < numTriangles; ++triIDX)
        {
            ...
            if (result.triangles0.find(triID0) == result.triangles0.end())
            {
                result.triangles0.insert(triID0);
                ...
                result.intersectingFaces0.insert(faceID0);
            }

            if (result.triangles1.find(triID1) == result.triangles1.end())
            {
                result.triangles1.insert(triID1);
                ...
                result.intersectingFaces1.insert(faceID1);
            }
        }
    }
};
...
public:
    ...
    void writeBundleIndicesArray(omni::graph::core::ogn::OmniGraphDatabase& db,
                                 ...
                                 const std::unordered_set<uint32_t>& facesIndicesSet)
    {
        ...
        facesIndices->resize(facesIndicesSet.size());
        uint32_t idx = 0;
        auto faceData = facesIndices->data();
        for (auto face : facesIndicesSet)
        {
            faceData[idx] = face;
            idx++;
        }
