    ...
    async def _auto_reverse_device_input(self, driveModeIn, useGamepad):
        ...
        class KeyboardInjector:
            ...
            def register_to_update_loop(self):
                self._updateLoopSubs = omni.kit.app.get_app().get_update_event_stream().create_subscription_to_pop(
                    self._update, name="omni.physx.vehicle.test: test_auto_reverse")
...
class PhysXVehicleTestMemoryStage(PhysicsMemoryStageBaseAsyncTestCase, PhysXVehicleTestBase):
    ...
    async def _suspension_scenario(self, attr):
        ...
        for i in range(stepsToRun):
            for j in range(vehicleCount):
                gPhysXInterface.apply_force_at_pos(vehiclePaths[j], Gf.Vec3f(0.0, -force, 0.0), 
    ...
    async def test_tire_sticky_params_longitudinal(self):
        ...
        global gPhysXInterface
        for i in range(vehicleCount):
            gPhysXInterface.apply_force_at_pos(vehiclePaths[i], Gf.Vec3f(0.0, 0.0, force), 
                startPos[i], "Force")
    ...
    async def test_tire_sticky_params_lateral(self):
        ...
        global gPhysXInterface
        for i in range(vehicleCount):
            gPhysXInterface.apply_force_at_pos(vehiclePaths[i], Gf.Vec3f(force, 0.0, 0.0), 
                startPos[i], "Force")
