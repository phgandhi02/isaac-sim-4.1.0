...
class ConvexMeshDatademo(demo.Base):
    ...
    description = """
Demo showing convex mesh data usage. 
The convex mesh data from cooking can be obtained in or out of simulation.

- A single convex hull is overlapped with yellow outlines on a simple convex mesh using debug draw.
- Multiple convex hulls are created as Usd meshes to display the convex decomposition of the torus object.
    """
    ...
    def create_torus_decomposition(self):
        ...
        torus_xform.ClearXformOpOrder()
        torus_xform.AddTranslateOp().Set(Gf.Vec3f(100, 100, 0))
